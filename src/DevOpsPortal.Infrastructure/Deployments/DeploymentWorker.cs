...
/// <summary>
/// Background job runner: dequeues one deployment ID at a time and executes
    ...
    ILogger<DeploymentWorker> logger) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            try
            {
                deploymentId = await queue.DequeueAsync(stoppingToken);
            }
            ...
            try
            {
                await executor.ExecuteAsync(deploymentId, stoppingToken);
            }
            catch (Exception ex)
            {
                ...
                logger.LogError(ex, "Unhandled exception executing deployment {DeploymentId}", deploymentId);
            }
        }
    }
}
